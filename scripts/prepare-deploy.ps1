...
Write-Host "=== Cuty Expenses - deploy preflight ===" -ForegroundColor Cyan
Write-Host ""
...
if (Test-Path ".env") {
    ...
    Write-Host "[INFO] No local .env - build still works; runtime secrets live on VPS at /opt/cuty-expenses/.env" -ForegroundColor DarkGray
}

Write-Host ""
Write-Host "Isolation reminders:" -ForegroundColor DarkGray
Write-Host "  VPS path     : /opt/cuty-expenses (NOT /opt/cuty-platform)" -ForegroundColor DarkGray
Write-Host "  Subdomain    : expenses.cuty.center" -ForegroundColor DarkGray
Write-Host "  Docker stack : cuty-expenses (no host 80/443; joins cuty-platform network for TLS nginx only)" -ForegroundColor DarkGray
Write-Host ""
...
Write-Host ""
Write-Host "First deploy: copy env.production.expenses.example to /opt/cuty-expenses/.env on the VPS." -ForegroundColor DarkGray
Write-Host ""

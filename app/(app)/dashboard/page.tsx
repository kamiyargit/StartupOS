...
export default function DashboardPage() {
  ...
  const [yearScope, setYearScope] = useState<JalaliYearScope>(now.year);
  ...
  useEffect(() => {
    Promise.all([fetch("/api/cost-types"), fetch("/api/users")]).then(async ([t, u]) => {
  ...
  const barData = monthlyCurrency === "TOMAN" ? barDataToman : barDataUsd;
  const barFill = monthlyCurrency === "TOMAN" ? chartColors.barFill : chartColors.barFillUsd;
  const hasChartData = barData.some((item) => item.amount > 0);
  const chartTitle = chartMode === "years" ? "هزینه سالانه" : "هزینه ماهانه";

  return (
    <div className="min-w-0 space-y-6">
      <div>
        <h1 className="text-xl font-bold text-slate-900 dark:text-gh-fg sm:text-2xl">داشبورد مالی</h1>
        <p className="text-sm text-slate-500 dark:text-gh-fg-muted">نمای کلی هزینه‌های سازمان</p>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent className="space-y-1">
            <p className="text-2xl font-bold text-emerald-700 dark:text-gh-success" dir="ltr">
              {formatExpenseMoney(stats?.periodAmountToman ?? 0, "TOMAN")}
            </p>
            {Number(stats?.periodAmountUsd ?? 0) > 0 && (
              <p className="text-lg font-semibold text-emerald-600 dark:text-gh-success" dir="ltr">
        ...
        <CardContent className="space-y-2">
          {!stats?.financierTotals?.length ? (
            ...
            stats.financierTotals.map((item) => (
              <div
                key={item.userId}
                className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-slate-200 px-3 py-2 text-sm dark:border-gh-border"
              >
